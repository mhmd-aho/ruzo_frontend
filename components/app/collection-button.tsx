...
export default function CollectionButton({ onClose }: CollectionButtonProps) {
        ...
        <div className="h-fit lg:h-full max-lg:w-full flex flex-col items-center">
            ...
            {isHovered && (
                <div  className="lg:absolute lg:top-20 lg:left-0 lg:bg-black/20 lg:backdrop-blur-sm w-full lg:h-screen h-fit">
                    <div onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className="flex flex-col items-start justify-start gap-6 bg-white w-full h-72 lg:p-9 p-3 overflow-auto">
                        ...
                        <div className="flex flex-wrap max-lg:flex-col justify-start max-lg:items-start lg:gap-8 gap-4 items-center">
                            <Link onClick={() => { setIsHovered(false); onClose?.(); }} className="text-mid text-sm lg:text-xl " href='/collections'>All</Link>
                            {categories.map((category) => (
                                <Link key={category.id} onClick={() => { setIsHovered(false); onClose?.(); }} className="text-mid text-sm lg:text-xl " href={`/collections/?category=${category.name}`}>{category.name}</Link>
                            ))}
                        </div>
                    </div>
                </div>
